app.controller("squp.simplemdecontroller", function ($scope, $log, $timeout, assetsService, dialogService, contentResource) {
	...
	**/
    assetsService.load(["/app_plugins/simplemde-markdown-editor/lib/simplemde.min.js"]).then(function () {
        ...
        var editor = new SimpleMDE({
            ...
            toolbar: [
                ...
                {
                    name: "internal-link",
                    action: $scope.insertInternalLink,
    ...
    /**
		Use Umbraco's media picker to choose an image
	**/
    ...
    /**
        Use Umbraco's content picker to choose an node
    **/
    $scope.insertInternalLink = function (editor) {
        dialogService.treePicker({
            section: "content",
            callback: function (item) {
                var text = editor.codemirror.getSelection();
                contentResource.getNiceUrl(item.id).then(function (url) {
